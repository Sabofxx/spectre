...
def build_site(conn: sqlite3.Connection, out_dir: Path) -> dict[str, int]:
    ...
    # Flag events whose most recent article is under 45 min old, for a "nouveau"
    # marker on the feed. Kept tight so the marker stays meaningful.
    fresh_cut = (now - timedelta(minutes=45)).isoformat(timespec="seconds")
    for c in feed_cards:
        c["is_fresh"] = bool(c["latest_published"] and c["latest_published"] >= fresh_cut)
